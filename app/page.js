...
export default function Home() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
          setHistoryData(data.map(d => ({
              ...d,
              // WICHTIG: Konvertiere Strings in Zahlen für Recharts
    ...
    } catch (e) { console.error("History Error", e); setHistoryData([]); }
  }, [timeRange]);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-10">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Temp */}
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 relative overflow-hidden group hover:border-red-200 transition-colors">
             {/* FIX: Farbe für das Icon hinzugefügt */}
             <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><Thermometer size={120} className="text-red-500" /></div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 relative overflow-hidden group hover:border-blue-200 transition-colors">
             {/* FIX: Farbe für das Icon hinzugefügt */}
             <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><Droplets size={120} className="text-blue-500" /></div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col justify-center gap-3">
             ...
             <button 
                ...
                // FIX: Die Logik für die Farbe war korrekt, aber wir stellen sicher, dass der Status sauber geladen wird.
